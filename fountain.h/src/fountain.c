...
static const FN_MARKER_DEF MARKER_DEFS[] = {
    {"_***", "***_", 4, 4,
     ...
     0, CHAR_STAR | CHAR_UNDER},
    {"***", NULL, 3, 0, SPAN_BIT(FN_SPAN_STRONG) | SPAN_BIT(FN_SPAN_EMPHASIS),
     0, CHAR_STAR},
    {"_**", "**_", 3, 3, SPAN_BIT(FN_SPAN_STRONG) | SPAN_BIT(FN_SPAN_UNDERLINE),
     0, CHAR_STAR | CHAR_UNDER},
    {"_*", "*_", 2, 2, SPAN_BIT(FN_SPAN_EMPHASIS) | SPAN_BIT(FN_SPAN_UNDERLINE),
     0, CHAR_STAR | CHAR_UNDER},
...
typedef struct FN_CTX {
    ...
    FN_BLOCKTYPE
        last_block; /* type of the most recent lyrics block (for spacer) */
...
static int fn_text_(FN_CTX *ctx, FN_TEXTTYPE type, const FN_CHAR *s,
                    FN_SIZE len)
{
    if (ctx->parser->text)
...
static int fn_init_input_(FN_CTX *ctx, const FN_CHAR *input, FN_SIZE input_size)
{
    ...
    if (fn_grow_((void **)&ctx->lines, &ctx->alloc_lines, est_lines,
                 sizeof(FN_LINE)) != 0)
        return -1;
...
#define AT(i)                                                                  \
    ((beg + (i) < end) ? (char)fn_toupper_((UCHAR)s[beg + (i)]) : '\0')

#define IS_DELIM(i)                                                            \
    (beg + (i) < end && (s[beg + (i)] == '.' || s[beg + (i)] == '-' ||         \
...
static int fn_extract_scene_number_(const FN_CHAR *s, FN_OFFSET beg,
                                    FN_OFFSET *text_end, const FN_CHAR **num,
                                    FN_SIZE *num_size)
{
...
{
    if (fn_grow_((void **)&ctx->consumed, &ctx->alloc_consumed, size,
                 sizeof(unsigned char)) != 0)
        return -1;
    return 0;
}

static int fn_add_mark_(FN_CTX *ctx, FN_OFFSET pos, unsigned len, int is_opener,
                        unsigned span_bits)
{
    if (fn_grow_((void **)&ctx->marks, &ctx->alloc_marks, ctx->n_marks + 1,
                 sizeof(FN_MARK)) != 0)
        return -1;
...
{
    static const FN_SPANTYPE open_order[] = {FN_SPAN_UNDERLINE, FN_SPAN_STRONG,
                                             FN_SPAN_EMPHASIS, FN_SPAN_NOTE};
...
static int fn_process_inline_line_(FN_CTX *ctx, const FN_CHAR *s, FN_OFFSET beg,
                                   FN_OFFSET end)
{
    FN_SIZE len = end - beg;
    if (len == 0)
    ...
    for (unsigned m = 0; m < N_MARKER_DEFS; m++) {
        if ((MARKER_DEFS[m].required_chars & present) ==
            ...
            CHECK(fn_scan_marker_(ctx, line, len, ctx->consumed,
                                  &MARKER_DEFS[m]));
    }
...
static int fn_has_inline_markers_(const FN_CHAR *s, FN_OFFSET beg,
                                  FN_OFFSET end)
{
    for (FN_OFFSET i = beg; i < end; i++) {
        if (s[i] == '*' || s[i] == '_' || s[i] == '[')
...
static int fn_process_inline_(FN_CTX *ctx, FN_OFFSET beg, FN_OFFSET end)
{
    if (!fn_has_inline_markers_(ctx->text, beg, end)) {
        ...
        for (FN_OFFSET i = beg; i <= end; i++) {
            if (i == end || ctx->text[i] == '\n') {
                if (line_beg < i)
                    CHECK(fn_text_(ctx, FN_TEXT_NORMAL, ctx->text + line_beg,
                                   i - line_beg));
                if (i < end)
...
static int fn_group_add_(FN_GROUP *g, FN_BLOCKTYPE type, FN_OFFSET beg,
                         FN_OFFSET end, int is_dual)
{
...
static int fn_parse_title_page_(FN_CTX *ctx, unsigned *body_start)
{
    ...
    for (unsigned i = 0; i < blank_line; i++) {
        ...
        if (has_colon) {
            ...
            int is_author = (key_len == 6 && ctx->text[ln->beg] == 'a' &&
                             ctx->text[ln->beg + 1] == 'u' &&
                             ctx->text[ln->beg + 2] == 't' &&
                             ctx->text[ln->beg + 3] == 'h' &&
                             ctx->text[ln->beg + 4] == 'o' &&
...
#define NO_LOWER_()                                                            \
    (no_lower >= 0 ? no_lower                                                  \
        ...
        if (llen > 0 && ctx->text[lbeg] == '~') {
            ...
            CHECK(fn_emit_block_(ctx, FN_BLOCK_LYRICS, NULL, lbeg + 1, lend));
        ...
        if (llen > 0 && ctx->text[lbeg] == '!') {
            ...
            CHECK(fn_emit_block_(ctx, FN_BLOCK_ACTION, &ad, lbeg + 1, lend));
        ...
        if (llen > 0 && ctx->text[lbeg] == '@') {
            ...
            CHECK(fn_complete_curr_group_(ctx));
            fn_group_clear_(&ctx->curr_group);
            if (fn_group_add_(&ctx->curr_group, FN_BLOCK_CHARACTER, lbeg + 1,
                              lend, 0) != 0)
                return -1;
        ...
        if (ctx->in_dialogue && fn_is_ws_of_len_(ctx->text, lbeg, lend, 2, 1)) {
            ...
            if (ctx->curr_group.n > 0) {
                ...
                if (last->type == FN_BLOCK_DIALOGUE) {
                    ...
                    if (fn_group_add_(&ctx->curr_group, FN_BLOCK_DIALOGUE, lbeg,
                                      lend, 0) != 0)
                        return -1;
                }
            }
        ...
        if (llen >= 2 && ctx->text[lbeg] == '/' && ctx->text[lbeg + 1] == '*') {
            if (fn_ends_with_boneyard_close_(ctx->text, lbeg, lend)) {
                ...
                if (ctx->flags & FN_FLAG_BONEYARD) {
                    ...
                    CHECK(fn_emit_block_plain_(ctx, FN_BLOCK_BONEYARD, NULL, tb,
                                               te));
                }
        ...
        if (newlines_before > 0) {
            ...
            if (tend - tbeg >= 4 && ctx->text[tbeg] == '[' &&
                ctx->text[tbeg + 1] == '[' && ctx->text[tend - 1] == ']' &&
                ctx->text[tend - 2] == ']') {
            ...
            if (tbeg < tend && ctx->text[tbeg] == '#') {
                ...
                if (text_beg < tend) {
                    if (ctx->flags & FN_FLAG_SECTIONS) {
                        ...
                        sd.depth = depth;
                        CHECK(fn_emit_block_plain_(ctx,
        ...
        if (llen > 1 && ctx->text[lbeg] == '.' && ctx->text[lbeg + 1] != '.') {
            ...
            fn_extract_scene_number_(ctx->text, text_beg, &text_end,
                                     &sd.scene_number, &sd.scene_number_size);

            CHECK(fn_emit_block_(ctx, FN_BLOCK_SCENE_HEADING, &sd, text_beg,
                                 text_end));
        ...
        if (newlines_before > 0 &&
            ...
            CHECK(fn_emit_block_(ctx, FN_BLOCK_SCENE_HEADING, &sd, lbeg,
                                 text_end));
            ctx->last_block = FN_BLOCK_SCENE_HEADING;
            newlines_before = 0;
            ctx->in_dialogue = 0;
        ...
        if (newlines_before > 0) {
            ...
            if (tlen == 9 && memcmp(ctx->text + tbeg, "FADE OUT.", 9) == 0)
                is_known = 1;
                     ...
                     memcmp(ctx->text + tbeg, "CUT TO BLACK.", 13) == 0)
                is_known = 1;
            else if (tlen == 14 &&
                     memcmp(ctx->text + tbeg, "FADE TO BLACK.", 14) == 0)
                is_known = 1;

            if (is_known) {
                CHECK(fn_flush_all_dialogue_(ctx));
                CHECK(
                    fn_emit_block_(ctx, FN_BLOCK_TRANSITION, NULL, lbeg, lend));
                ctx->last_block = FN_BLOCK_TRANSITION;
                newlines_before = 0;
                ctx->in_dialogue = 0;
                continue;
        ...
        if (llen > 0 && ctx->text[lbeg] == '>') {
            ...
            if (llen > 1 && ctx->text[lend - 1] == '<') {
                ...
                ad.is_centered = 1;
                CHECK(fn_emit_block_(ctx, FN_BLOCK_ACTION, &ad, text_beg,
                                     text_end));
            } else {
                /* Forced transition. */
                FN_OFFSET text_beg = lbeg + 1;
                FN_OFFSET text_end = lend;
                fn_trim_(ctx->text, &text_beg, &text_end);
                CHECK(fn_emit_block_(ctx, FN_BLOCK_TRANSITION, NULL, text_beg,
                                     text_end));
            }
        ...
        if (newlines_before > 0 && NO_LOWER_() &&
            ...
            if (i + 1 < ctx->n_lines &&
                ...
                if (trimmed_end > lbeg && ctx->text[trimmed_end - 1] == '^') {
                    is_dual = 1;
        ...
        /* ----- Action (fallback) ----- */
        {
            ...
            FN_BLOCK_ACTION_DETAIL ad = {0};
            CHECK(
                fn_emit_block_(ctx, FN_BLOCK_ACTION, &ad, text_beg, text_end));
            ctx->last_block = FN_BLOCK_ACTION;
            newlines_before = 0;
            ctx->in_dialogue = 0;
        }
